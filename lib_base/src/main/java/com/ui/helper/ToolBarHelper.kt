...
class ToolBarHelper {
    companion object {
        ...
        fun setToolBar(activity: AppCompatActivity, appbar: AppBarLayout, toolbar: Toolbar, appBarColor: Int, toolBarColor: Int) {
            val barHeight = getStatusBarHeight(activity)
            appbar.setBackgroundColor(appBarColor)
            toolbar.setBackgroundColor(toolBarColor)
            activity.setSupportActionBar(toolbar)
            activity.supportActionBar?.setDisplayHomeAsUpEnabled(true) // 设置返回按钮可见
